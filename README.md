...
This repository contains eight OpenWrt packages. It builds normal `.apk`
...
- `apn-autoconfig-modem`, optional modem inventory, identity, ownership and
...
apn-autoconfig desired-bearer-state --target network:<section>
apn-autoconfig target-policy --target network:<section>
apn-autoconfig-modem attachment --modem <modem-id>
```

Since 0.15.3 the status schema also says whether the last result is about the
modem and SIM that are present now. `result_state` is `current`, `previous` or
`unknown`; `result_scope` names the attachment generation the result was written
for and whether a SIM identifier was known at the time — never the identifier
itself — and `result_stale_reason` says which comparison decided it. `previous`
and `unknown` are different states and neither is an error: nothing failed, the
last thing that succeeded was simply about something else. A result stored by
0.15.2 or earlier reads as `unknown`, and the first reconcile per target
replaces it.

A modem that is unplugged and returned now converges on its own. The debounced
hotplug scan stays read-only; a separate worker reconciles only the targets
whose own modem came back, so reconnecting one modem never cycles another. An
interface you stopped with Disconnect stays stopped across a detach and
reattach — that intention is recorded per target and is written only by the
connect and disconnect controls and by provisioning's requested autoconnect.
...
config apn_autoconfig 'main'
        ...
        option read_deadline_seconds '20'
```

`read_deadline_seconds` bounds the **total** a read the LuCI page is waiting for
may spend — `status-json`, `detect-json` and `targets-json`. It exists because
rpcd's `file exec`, the transport under every LuCI read, sends `SIGKILL` after
its own timeout (30 seconds by default): no trap covers that, so the page loses
the whole response and the helper's children are orphaned. Each backend call is
individually bounded and nothing bounded their sum, and the sum is what rpcd
measures.

Within the deadline each call is given whatever time is left rather than its own
full budget, and a call with no time left is not started at all. If the deadline
is reached, the command still prints a valid document containing everything it
did read, adds `incomplete` and `incomplete_reads` naming what it gave up on,
and exits 3 (retryable). The LuCI page renders that as an incomplete reading
with a retry, never as absent hardware. Nothing here reports a modem, SIM,
registration or capability it did not actually read.

The value must be a positive whole number of seconds **below** rpcd's own
timeout; a larger one is rejected rather than obeyed, because a deadline at or
above the caller's budget cannot keep the command inside it. Lower it if rpcd is
configured with a shorter timeout than the default. Mutations — `reconcile`,
`apply`, `modem-reset` — are not affected: nobody's browser is waiting on them.
...
This repository carries what a user or a licence needs: the packages, the
source that is actually installed, the vendored LPA with its patches and
notices, licence texts, this document and the changelog.

Development happens in a private repository. The architecture documents,
contracts, test suites, release test plans and hardware records are kept there
and are not published.

The independently versioned provider database is maintained here as a narrow

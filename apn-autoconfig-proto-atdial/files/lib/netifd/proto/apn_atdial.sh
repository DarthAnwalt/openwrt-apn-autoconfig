...
# lands in `logread` exactly as one from `apn-autoconfig-modem` would. Same
# convention as the helper of that name there; duplicated because a netifd
# protocol handler sources nothing from the modem package at runtime.
atdial_mask_modem_id() {
	local prefix rest
	case "${1:-}" in
		'') printf '<unknown>' ; return 0 ;;
		imei:*) prefix='imei:'; rest="${1#imei:}" ;;
		usb-serial:*) prefix='usb-serial:'; rest="${1#usb-serial:}" ;;
		*) printf '%s' "$1" ; return 0 ;;
	esac
	if [ "${#rest}" -le 4 ]; then
		printf '%s...' "$prefix"
	else
		printf '%s...%s' "$prefix" "${rest#"${rest%????}"}"
	fi
}
...
proto_apn_atdial_setup() {
	...
	if [ -n "$usbpath" ] && ! atdial_usb_present "$usbpath"; then
		if [ -n "$modem_id" ]; then
			...
			if [ -n "$value" ] && atdial_usb_present "$value"; then
				atdial_log "usbpath $usbpath is no longer on the bus; using $(atdial_mask_modem_id "$value") for $(atdial_mask_modem_id "$modem_id")"
				usbpath="$value"
			fi
		fi
	fi
	...
	if [ -z "$dial" ] && [ -n "$modem_id" ] && [ -x "$ATDIAL_MODEM_BIN" ]; then
		...
		if [ "$ATDIAL_BOUND_MODEM_ID" != "$modem_id" ]; then
			atdial_log "modem_id \"$(atdial_mask_modem_id "$modem_id")\" was superseded at $usbpath by $(atdial_mask_modem_id "$ATDIAL_BOUND_MODEM_ID")"
		...
		if [ "$port_status" -eq 3 ] && [ -n "$usbpath" ]; then
			...
			if [ -n "$value" ] && [ "$value" != "$modem_id" ]; then
				atdial_log "modem_id \"$(atdial_mask_modem_id "$modem_id")\" no longer resolves; $usbpath is now $(atdial_mask_modem_id "$value")"
				modem_id="$value"
				port_status=0
				dial="$("$ATDIAL_MODEM_BIN" at-port --modem "$modem_id" 2>/dev/null)" || port_status=$?
			fi

...
/* An interface has two quite different reasons to be down, and 0.15.3 is the
 * release that makes the difference matter: convergence deliberately leaves a
 * target alone when somebody pressed Disconnect, so replugging the modem no
 * longer brings it back — and a user who is not told that has no way to work
 * out why. `interface_up` alone renders both reasons identically. Absent, as
 * from an older backend, reads as `auto`, which is what it has always meant. */
function connectionLabel(status) {
	if (status && status.interface_up)
		return _('up');
	return status && status.desired_bearer_state === 'down'
		? _('down — disconnected on purpose')
		: _('down or pending');
}

function connectionIsDeliberatelyDown(status) {
	return !!(status && !status.interface_up && status.desired_bearer_state === 'down');
}
...
return view.extend({
	...
	stripNodes: function(status, action) {
		...
		items.push(item(_('Registration'), registrationLabel(status),
			status.registration_state === 'denied' || status.registration_state === 'emergency-only'
				? 'apn-strip-bad' : ''));
		/* An interface someone deliberately stopped is not a problem, so it
		 * does not get the colour that says one. */
		items.push(item(_('Connection'), connectionLabel(status),
			status.interface_up ? 'apn-strip-good'
				: connectionIsDeliberatelyDown(status) ? '' : 'apn-strip-warn'));

		/* Three states, three presentations, and none of them an error.
		 *
		 * `previous` carries information a user can act on — a verdict that was
		 * true about something else. `unknown` invites them to run something
		 * instead. Collapsing the two into one greyed-out line throws that
		 * difference away, and colouring either of them like a failure claims
		 * that something went wrong when nothing did. */
		var state = resultState(status);
		if (!status.last_result)
			items.push(item(_('Last result'), _('nothing recorded yet'), 'apn-strip-stale'));
		else if (state === 'current')
			items.push(item(_('Last result'), status.last_result,
				status.result_code && status.result_code !== 'success' ? 'apn-strip-bad' : ''));
		else if (state === 'previous')
			items.push(item(_('Previous result'), status.last_result, 'apn-strip-stale'));
		else
			items.push(item(_('Result, unconfirmed'), status.last_result, 'apn-strip-unknown'));

		/* Not an error and not silence. The strip is what is visible from every
		 * area, so this is where "part of what you are reading is missing"
		 * belongs; the areas themselves say which part and offer the retry. */
		if (status.incomplete === true)
			items.push(item(_('Readings'), _('some could not be completed in time'),
				'apn-strip-unknown'));

		var running = this.runningDescription(action);
		if (running)
			items.push(item(_('Running'), running, 'apn-strip-busy'));
	...
	modemOperationText: function(operation) {
		...
		if (operation.busy) {
			if (operation.action === 'converge') {
				...
				return stage
					? '%s %s'.format(_('This modem reconnected and is being caught up.'), stage)
	...
	radioTable: function(status) {
		return table([
			...
			row(_('Mobile interface'), '%s: %s'.format(status.interface, connectionLabel(status)),
				connectionIsDeliberatelyDown(status)
					? _('This interface is down because it was disconnected here, not because anything failed. Reconnecting the modem will not bring it back; press Connect when you want it again.')
					: null)
		]);
	},
	...
	apnAreaNodes: function(status) {
		...
		var nodes = this.incompleteNotice(status).concat(this.statusWarnings(status));
		nodes.push(table([
	...
	/* The second place a result appears. The strip has room for a label and a
	 * value; here there is room to say why a verdict is not about the present,
	 * which is the part that tells the user whether to do anything about it. */
	lastResultRow: function(status) {
		var state = resultState(status);
		if (!status.last_result || state === 'current')
			return row(_('Last result'), status.last_result);

		var reason = resultStaleReasonText(status.result_stale_reason);
		var label = state === 'previous' ? _('Previous result') : _('Result, unconfirmed');
		var explanation = state === 'previous'
			? _('This is what happened last time, and it was about a different modem or SIM.')
			: _('This was recorded earlier, and nothing here can confirm it is about the modem and SIM present now.');
		var help = state === 'previous'
			? _('A result stops being current when the modem is reconnected or the SIM changes. It is kept because it is still what happened, not because it still applies.')
			: _('Not an error. Re-detecting the APN replaces it with a result about what is here now.');

		return row(label, E('div', { 'class': 'apn-result-' + state }, [
			E('div', {}, [ status.last_result ]),
			E('div', { 'class': 'apn-result-note' }, [
				reason ? '%s — %s'.format(explanation, reason) : explanation
			])
		]), help);
	},

	/* What an abandoned read looks like to a user.
	 *
	...
	incompleteNotice: function(status) {
		...
		/* The button is held by name rather than reached through the event, so
		 * disabling it while the read runs does not depend on which node the
		 * click happened to land on. */
		var retry = E('button', {
			...
			'click': function(ev) {
				...
				if (retry.disabled)
					return;
				retry.disabled = true;
				self.refreshPanels().catch(function() {}).then(function() {
		...
		return [ E('div', { 'class': 'alert-message notice apn-incomplete' },
			messages.map(function(message) { return E('p', {}, [ message ]); }).concat([
	...
	simAreaNodes: function(status) {
		...
		return this.incompleteNotice(status).concat([
			table([
	...
	/* One line for the strip, whichever component is busy. The engine owns the
	 * operations a user starts; the coordinator owns the one that starts
	 * itself, and before it reaches the engine's reconcile there is nothing in
	 * the engine's own state to show. Without this the page showed no running
	 * operation at all during that stretch, while the modem was being worked
	 * on. */
	runningDescription: function(action) {
		if (action && !action.error && action.busy)
			return this.actionDescription(action);

		var modems = (this.modemInventory && Array.isArray(this.modemInventory.modems))
			? this.modemInventory.modems : [];
		for (var i = 0; i < modems.length; i++) {
			var operation = modems[i].operation;
			if (!operation || operation.error || !operation.busy)
				continue;
			if (operation.action !== 'converge')
				return _('The %s is running.').format(this.actionLabel(operation.action));
			var stage = this.convergeStageText(operation.message);
			return stage
				? '%s %s'.format(_('The modem reconnected; the program is catching up.'), stage)
				: _('The modem reconnected; the program is catching up.');
		}
		return '';
	},
		...
		return m.render().then(function(mapNode) {
			...
			var page = E('div', { 'class': 'apn-autoconfig-page' }, [
				E('style', { 'type': 'text/css' }, [
					...
					/* Neither of the two demoted result states is an error, so
					 * neither borrows the failure colour. They are told apart
					 * from each other as well: a previous result is settled and
					 * merely old, an unconfirmed one is a question. */
					'.apn-autoconfig-page .apn-strip-stale .apn-strip-value{opacity:.65;font-weight:400}' +
					'.apn-autoconfig-page .apn-strip-unknown .apn-strip-value{opacity:.65;font-weight:400;font-style:italic}' +
					'.apn-autoconfig-page .apn-result-note{margin-top:.25rem;font-size:90%;opacity:.75}' +
					'.apn-autoconfig-page .apn-result-previous>div:first-child{opacity:.7}' +
					'.apn-autoconfig-page .apn-result-unknown>div:first-child{opacity:.7;font-style:italic}' +
